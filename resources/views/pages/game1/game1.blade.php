...
<div class="answer">
    <div class="row">
        <div class="col-sm-9 col-xs-12">
            <div class="row">
                <div style="text-align: center" class="col-6 f" id="{{$tuvung3['id']}}"> 
                <img   src="{{ asset('upload/image/'.$tuvung3->hinhanh) }}"></div>
                        @php $tukhac1= $chude->tuvung->whereNotIn('id', $tuvung3['id']);
                        $tukhac = $tukhac1->shuffle()->take(3); @endphp
                        @foreach($tukhac as $item)

                <div style="text-align: center"  class="col-6 f " style="color:red" id=" {{ $item['id'] }}"> 
                <img    src="{{ asset('upload/image/'.$item->hinhanh) }}"></div>
